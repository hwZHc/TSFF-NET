...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        max_pool = self._max_pooling(x).view(x.size(0), -1)
        num_pool = avg_pool+max_pool
        
        # Fully connected layer
        channel_weights = self.fc(num_pool).view(x.size(0), x.size(1), 1, 1)
        
        # Applying channel-wise attention
        x = x + x * channel_weights.expand_as(x)
        return x
...
class ImageEmbedder(nn.Module):
    def __init__(
        ...
        self.to_patch_embedding = nn.Sequential(
            ...
            Rearrange('b (c p3) (h p1) (w p2) -> b (p1 p2 c) (h w p3)', p1 = patch_size, p2 = patch_size, p3 = 1),
        ...
        # self.to_patch_embedding = nn.Sequential(
        #     #nn.Conv2d(1280, dim, kernel_size=1),
        #     Rearrange('b c (h p2) (w p3) -> b (h w) (p2 p3 c)', p2=patch_size,
        #               p3=patch_size), 
        #     #nn.Linear(patch_dim, dim),
        # )
        #self.patch_to_embedding = nn.Conv1d(in_channels=1, out_channels=num_patches, kernel_size= channels, stride=channels)
        #self.conv1 = nn.Conv2d(1280, 320, kernel_size=3, stride=1, padding=1)
        self.channel_attention = ChannelAttention(320)
        # self._avg_pooling = nn.AdaptiveAvgPool2d(1)
        # self._max_pooling = nn.AdaptiveMaxPool2d(1)
        # self._fc = nn.Linear(1280, 320)
        # self._fc_out = nn.Linear(320,1280)
    ...
    def forward(self, img):
        ...
        #x = self.efficient_net.extract_endpoints(img)
        #print(x.shape)
...
class CrossEfficientViT(nn.Module):
    def __init__(
        ...
        self.sm_image_embedder = ImageEmbedder(dim=sm_dim, image_size=224, patch_size=1,
                                               dropout=emb_dropout, efficient_block=16, channels=sm_channels)
        self.lg_image_embedder = ImageEmbedder(dim=lg_dim, image_size=224, patch_size=1,
                                               dropout=emb_dropout, efficient_block=16, channels=lg_channels)
