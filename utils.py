...
def custom_video_round(preds):
    preds_ = [pred.item() for pred in preds]
    #preds_ = preds
    m = max(preds_)
    if m > 0.6:
        return m
    return sum(preds_)/len(preds_)
    
def check_correct0(preds, labels):
    preds = preds.cpu()
    labels = labels.cpu()
    preds = [np.asarray(torch.sigmoid(pred).detach().numpy()).round() for pred in preds]
    ...
    for i in range(len(labels)):
        ...
        if pred == 1:
            ...
            negative_class += 1
    return correct, positive_class, negative_class,labels[0]

def check_frame_every(preds):
    N_0 = 0
    N_1 = 0
    for i in preds:
        if i.item() < 0.55:
    ...
    if N_1 == 0 or N_0 == 0:
        ...
        pred = 0.5*(1+sum(preds)/len(preds))
    return pred
